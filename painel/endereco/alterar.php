  ...
  try {
    ...
    if ($_FILES['fotoCliente']['error'] === UPLOAD_ERR_OK) {
      ...
      if (move_uploaded_file($_FILES['fotoCliente']['tmp_name'], $uploadFile)) {
        ...
        if ($id_endereco) {
          // Update the address
          $stmt = $pdo->prepare("UPDATE endereco SET rua = ?, numero = ?, bairro = ?, tipo_endereco = ?, complemento = ?,id_pessoa = ?, cep = ?, cidade = ?, uf = ?, img_comprovante_end = ?, dir_imagem = ? WHERE id_endereco = $id_endereco");
          ...
          if ($stmt->execute()) {
            ...
            $response = array(
              'success' => false,
              'message' => 'Erro ao atualizar o endereço no banco de dados.'
            );
          }
        } else {
          $response = array(
            'success' => false,
            'message' => 'O endereço não existe no banco de dados. A atualização não será realizada.'
          );
        }
      } else {
        $pdo->rollBack(); // Desfaz a transação
